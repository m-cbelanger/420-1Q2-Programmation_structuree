...
> Pour que les accents de la langue française apparaissent sous Windows, on ajoute quelques lignes de code à des endroits précis. En Linux, ça n'est pas obligatoire. (fortement recommandé de le mettre si vous voulez que la sortie soit identique sur les postes Linux et Windows)
...
| `/`     | Division                        |
| `%`     | Modulo                          |
| `pow(base,exposant)`     | exposant (avec un include cmath)                       |
...
4. Exécution
    ...
    - Code::Blocks lance l’exécutable (.exe).
